...
var YPC = YPC || {};
YPC.UI = YPC.UI || {};
YPC.VIEWPORT_WIDTH = null;
YPC.VIEWPORT_HEIGHT = null;
YPC.IS_MOBILE = false;
YPC.IS_VIEWTYPE = null;
YPC.IS_IE8 = ( $('html').hasClass('ie8') ) ? true : false;
YPC.IS_SIZE = YPC.IS_SIZE || {};
YPC.IS_SIZE.MAXMOBILE = 768;
YPC.IS_SIZE.MAXTABLET = 1023;
YPC.IS_TABLET = false;
YPC.FOCUS_ELEM = null;

YPC.DELAY_FUNC = (function(){
    var timer = 0;
...
var SetViewSize = function(){
    YPC.VIEWPORT_WIDTH = Math.max(document.documentElement.clientWidth, window.innerWidth || 0);
    YPC.VIEWPORT_HEIGHT = Math.max(document.documentElement.clientHeight, window.innerHeight || 0);
};
...
var DeivceChkFn= function() {
    if(YPC.VIEWPORT_WIDTH < YPC.IS_SIZE.MAXMOBILE && YPC.IS_MOBILE){
        YPC.IS_VIEWTYPE = 'mobile';
    } else if(YPC.VIEWPORT_WIDTH <= YPC.IS_SIZE.MAXTABLET && YPC.IS_MOBILE){
        YPC.IS_VIEWTYPE = 'tablet';
    } else {
        if(YPC.VIEWPORT_WIDTH < YPC.IS_SIZE.MAXMOBILE ) {
            YPC.IS_VIEWTYPE = 'mobile';
        } else if (YPC.VIEWPORT_WIDTH <= YPC.IS_SIZE.MAXTABLET ) {
            YPC.IS_VIEWTYPE = 'tablet';
        } else {
            YPC.IS_VIEWTYPE = 'web';
        }
...
YPC.UI.ResponsiveImageSrc = function($elems, device){
    "use strict";
    var _this = this;
    _this.elem = $elems;
    _this.initFlag = true;
